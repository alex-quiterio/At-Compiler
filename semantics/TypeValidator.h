#ifndef __AT_SEMANTICS_TYPE_VALIDATOR_H__
#define __AT_SEMANTICS_TYPE_VALIDATOR_H__
...
class TypeValidator : public virtual SemanticProcessor {
   ...
   std::vector<FunctionSymbol*> &_context;
   bool _errors;
   int *level;

  public:
    TypeValidator(std::ostream &os, cdk::semantics::SymbolTable<Symbol> &symtab, std::vector<FunctionSymbol*> &context, int* lvel)
      : SemanticProcessor(os), _symtab(symtab), _context(context), _errors(false), level(lvel) {}
  public:
    ~TypeValidator();
    public:
   ...
   virtual void processAnyTypeExpression (cdk::node::expression::BinaryExpression *const node, int lvl);
   virtual void processIntOnlyExpression (cdk::node::expression::BinaryExpression *const node, int lvl);
   virtual void processIntOrRealExpression(cdk::node::expression::BinaryExpression *const node, int lvl);
  ...
  protected:
   ...
   virtual void processIntLogicalExpression(cdk::node::expression::BinaryExpression *const node, int lvl);
